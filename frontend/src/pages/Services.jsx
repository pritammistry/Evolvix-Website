...
export default function Services() {
  ...
  return (
    <section className="section page-section" data-testid="services-page">
      ...
      <div className="service-grid" data-testid="creative-services-grid">{creative.map((service) => <Link to={`/contact?service=${encodeURIComponent(service.title)}`} className="service-card" key={service.title} data-testid={`creative-service-${service.title.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`}><h3>{service.title}</h3><p>{service.text}</p><span className="service-card-go">Enquire <ArrowRight size={15} /></span></Link>)}</div>
      <SectionHeader eyebrow="AI + Technology" title="AI Business Consulting & Technology Solutions" />
      <div className="service-grid" data-testid="technology-services-grid">{tech.map((service) => <Link to={`/contact?service=${encodeURIComponent(service.title)}`} className="service-card" key={service.title} data-testid={`technology-service-${service.title.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`}><h3>{service.title}</h3><p>{service.text}</p><span className="service-card-go">Enquire <ArrowRight size={15} /></span></Link>)}</div>
