...
import { adminLogin, deleteProductFile, fetchAdminAnalytics, fetchAdminAnalyticsOptions, fetchAdminDashboard, resetAdminContent, saveAdminContent, saveAdminList, uploadProductFile } from "../api";
...
function CatalogEditor({ title, icon: Icon, items, onChange, kind, token, onRefresh }) {
  ...
  const fields = kind === "products" ? ["title", "slug", "category", "tag", "price", "description", "external_purchase_url"] : kind === "portfolio" ? ["title", "category", "summary", "image"] : ["title", "slug", "category", "excerpt", "body", "date", "read_time"];
