...
const blankDemo = { title: "New Demo", industry: "Retail / Optical", description: "Demo description.", features: ["Feature 1", "Feature 2"], url: "", primary_label: "", secondary_url: "", secondary_label: "", note: "", icon_key: "monitor", status: "Coming Soon", visible: true };
...
function DemosEditor({ items, onChange }) {
  ...
  // Swap with the neighbour — this is the order demos appear on the Demos page
  // and in the homepage preview.
  const move = (index, direction) => {
    const target = index + direction;
    if (target < 0 || target >= demos.length) return;
    const next = demos.slice();
    [next[index], next[target]] = [next[target], next[index]];
    onChange(next);
  };
  return <section className="admin-editor-card" data-testid="demos-editor"><h3><Monitor size={20} /> Live Demos</h3><p style={{color:"var(--muted)",marginBottom:18}}>Manage demo cards on the Demos page. Set status to "Coming Soon" to show placeholder cards without a live link. Use the arrows to set the order they appear in — the homepage preview shows the first three in this exact order.</p>{demos.map((item, index) => <div className="admin-pair" key={`demo-${index}`}><div className="admin-reorder-bar" data-testid={`demo-reorder-${index}`}><span className="admin-reorder-pos">#{index + 1}</span><span className="admin-reorder-name">{item.title || "Untitled demo"}</span><button type="button" className="admin-mini-btn" onClick={() => move(index, -1)} disabled={index === 0} aria-label={`Move ${item.title || "demo"} up`} data-testid={`demo-move-up-${index}`}><ChevronUp size={15} /></button><button type="button" className="admin-mini-btn" onClick={() => move(index, 1)} disabled={index === demos.length - 1} aria-label={`Move ${item.title || "demo"} down`} data-testid={`demo-move-down-${index}`}><ChevronDown size={15} /></button></div><div className="admin-form-grid"><TextField label="Title" value={item.title} onChange={(value) => update(index, "title", value)} testId={`demo-title-${index}`} /><TextField label="Industry / Category" value={item.industry} onChange={(value) => update(index, "industry", value)} testId={`demo-industry-${index}`} /><TextField label="Description" value={item.description} onChange={(value) => update(index, "description", value)} testId={`demo-description-${index}`} multiline /><TextField label="Live URL (blank for Coming Soon)" value={item.url} onChange={(value) => update(index, "url", value)} testId={`demo-url-${index}`} /><TextField label="Main Button Label (blank = 'View Live Demo')" value={item.primary_label || ""} onChange={(value) => update(index, "primary_label", value)} testId={`demo-primary-label-${index}`} /><TextField label="Second Demo URL (optional — e.g. admin panel)" value={item.secondary_url || ""} onChange={(value) => update(index, "secondary_url", value)} testId={`demo-secondary-url-${index}`} /><TextField label="Second Button Label (blank = 'View Admin Panel')" value={item.secondary_label || ""} onChange={(value) => update(index, "secondary_label", value)} testId={`demo-secondary-label-${index}`} /><TextField label="Note under buttons (optional — e.g. demo login)" value={item.note || ""} onChange={(value) => update(index, "note", value)} testId={`demo-note-${index}`} /><label className="admin-field" data-testid={`demo-status-${index}-field`}><span>Status</span><select value={item.status || "Coming Soon"} onChange={(e) => update(index, "status", e.target.value)} data-testid={`demo-status-${index}`}><option>Live Demo</option><option>Coming Soon</option><option>Now Building</option></select></label><label className="admin-field" data-testid={`demo-icon-${index}-field`}><span>Icon</span><select value={item.icon_key || "monitor"} onChange={(e) => update(index, "icon_key", e.target.value)} data-testid={`demo-icon-${index}`}><option value="shopping">Shopping / Retail</option><option value="monitor">Monitor / Tech</option><option value="book">Book / Education</option><option value="food">Food / Restaurant</option><option value="health">Health / Clinic</option><option value="phone">Mobile / Services</option><option value="chart">Chart / Finance</option><option value="zap">Zap / Automation</option><option value="lock">Lock / Secure Delivery</option></select></label></div><ArrayEditor title="Features" items={item.features || []} onChange={(value) => update(index, "features", value)} placeholder="Feature" testPrefix={`demo-features-${index}`} /><label className="admin-check"><input type="checkbox" checked={item.visible !== false} onChange={(e) => update(index, "visible", e.target.checked)} data-testid={`demo-visible-${index}`} /> Visible on website</label><button type="button" className="danger-btn" onClick={() => onChange(demos.filter((_, i) => i !== index))} data-testid={`demo-delete-${index}`}><Trash2 size={16} /> Remove</button></div>)}<button type="button" className="admin-mini-btn" onClick={() => onChange([...demos, { ...blankDemo, id: `demo-${Date.now()}` }])} data-testid="demo-add-button"><Plus size={16} /> Add Demo</button></section>;
}

function PlaygroundEditor() {
  const BLANK = { category: "music", title: "", description: "", url: "", thumbnail: "", preview_url: "", visible: true };
  const [items, setItems] = useState([]);
  ...
  const handleEdit = (item) => { setForm({ category: item.category, title: item.title, description: item.description, url: item.url, thumbnail: item.thumbnail || "", preview_url: item.preview_url || "", visible: item.visible }); setEditId(item.id); };
  ...
  const moveItem = async (item, direction) => {
    ...
    const swapId = catItems[targetIdx].id;
    const flatCopy = [...items];
    const aPos = flatCopy.findIndex((i) => i.id === item.id);
    const bPos = flatCopy.findIndex((i) => i.id === swapId);
    [flatCopy[aPos], flatCopy[bPos]] = [flatCopy[bPos], flatCopy[aPos]];
    setItems(flatCopy);
    try { await reorderPlayground(flatCopy.map((i) => i.id)); }
